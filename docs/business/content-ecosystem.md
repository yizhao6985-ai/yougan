...
发布时可携带（来自作品 profile / 成稿）：

| 维度 | 字段示例 | 用途 |
|------|----------|------|
| 平台 | `platform` | 小红书、公众号等 |
| 内容形态 | `contentFormat` | 筛选与展示 |
| 话题分类 | `topicCategory` | 发现页分类 |
| 选题/类型 | `contentTopic`、`contentType` | 搜索与推荐 |
| 媒介 | `mediaType` | 图文/视频等 |
| 标签 | `hashtags` | 话题传播 |
| 封面与配图 | `coverUrl`、`images` | 列表展示 |

taxonomy 定义于 `apps/api/src/lib/discover-taxonomy.ts`，驱动发现页筛选项。

### 发布流程（用户视角）

1. 创作模式生成 `creation.body`。
2. Studio 出现「发布到有感」。
3. 首次发布创建 `Publication`（草稿或直接发布，依 API 逻辑）。
4. 已发布作品显示「在发现灵感查看」链接 → `/content/:slug`。
