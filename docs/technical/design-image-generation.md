...
api: 落库前              ▼
  applyAgentRunToWork ──▶ materializeAgentRunValues
                        ...
                        │
web: 展示                ▼
  content-preview / publication-article ──▶ PreviewBlockList + PreviewImageGallery
```
...
- `assemblePreview`：把通过的设计片段整合进 `preview.blocks`（`notes` 为 text block、`images` 为 image block），并清空 `pending_tasks`。
...
- `apps/api/src/services/materialize-production-draft-images.ts`
  - `materializeAgentRunValues` → `materializeWorkProductionImages`：同时处理 `production.preview.blocks` 与每个 `task.deliverable.images` 草稿。
...
- 落库时 `committedProduction` 会剥离 task 上的 staging 草稿字段（`deliverable/feedback/...`），最终图片留在 `preview.blocks`。
...
- `apps/web/src/components/preview-image-list.tsx`：`PreviewImageList` 缩略图列表，`onError` 显示占位、点击打开图册。
- `apps/web/src/components/preview-image-gallery.tsx`：`PreviewImageGallery` 全屏图册，支持左右切换、键盘 ←/→、打开原图、多图圆点导航。
- 渲染入口：`content-preview.tsx`（创作台预览）与 `content/publication-article.tsx`（发布详情）通过 `PreviewBlockList` 读取 `preview.blocks` 渲染。

## 五、类型与配置

| 位置 | 内容 |
|------|------|
| `packages/domain/src/models/work/production-draft.ts` | `ProductionDraftImage`（`url/alt/prompt/transient`），设计任务 deliverable 配图草稿 |
| `packages/domain/src/models/work/preview.ts` | `ImagePreviewBlock`（整合后的 preview block） |
