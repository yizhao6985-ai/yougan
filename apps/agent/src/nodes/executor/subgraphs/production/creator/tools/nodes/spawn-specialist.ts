...
import { DEPARTMENT_LABELS } from "#agent/lib/production/department-labels.js";
import { markDepartmentTaskPendingInspect } from "#agent/lib/production/mark-inspect.js";
import { mergeStagingPatches, patchStagingPreview, patchStagingProductionPlan } from "#agent/lib/staging-state.js";
import { getPlanSummary, type WorkPreview } from "@yougan/domain";
import {
  parseActiveTurnKind,
...
请输出该部门的专业交付物，用 Markdown 格式。`;
    ...
    const inspectPatch =
      markDepartmentTaskPendingInspect(state, pending, department, pipeline) ??
      {};

    return toolCommand(config, `${name} 已完成任务。`, mergeStagingPatches(
