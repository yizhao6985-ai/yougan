...
export function buildOpeningTopicSuggestionsPrompt(
  ...
  const existingWorkContext = [
    ...
    `当前作品方案：${blueprintSummary(blueprint)}`,
  ].join("\n");

  const hasPriorWork =
    blueprint.beats.length > 0 ||
    blueprint.constraints.length > 0 ||
    Boolean(blueprint.premise.trim()) ||
    Boolean(blueprint.spec.content_topic);
...
- 好例（具体）：「我想写转行前 3 个简历坑，面向 3–5 年想跳槽的职场人」（≤${MAX_NEXT_STEP_SUGGESTION_LENGTH} 字）
