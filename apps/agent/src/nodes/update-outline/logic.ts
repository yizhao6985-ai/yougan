/**
 * 大纲 agent：进入大纲模式且尚无条目时，根据当前 brief 生成内容大纲。
 */
...
import { OutlineResponseSchema } from "./schema.js";
import { resolveContentSpec } from "../../lib/content-spec.js";

export function shouldGenerateOutline(state: AgentStateType): boolean {
...
function buildOutlinePrompt(state: AgentStateType): string {
...
export async function runUpdateOutline(
  state: AgentStateType,
): Promise<Partial<AgentStateType>> {
  if (!shouldGenerateOutline(state)) {
    return {};
  }
  ...
  try {
    const parsed = await invokeStructuredOutput(
      ...
      [new HumanMessage(buildOutlinePrompt(state))],
      { name: "work_outline" },
    );
