...
import { buildOutlineFormatHint } from "./format-prompts.js";
...
export function buildCreationSystemPrompt(state: AgentStateType): string {
  ...
  const formatHint = buildOutlineFormatHint(
    profile.content_format as Parameters<typeof buildOutlineFormatHint>[0],
  );
...
你的任务是在已定稿创作大纲的框架下，按已确定的体裁与媒介形式生成或修改成稿。执行用户最新需求前，先把新需求并入待执行变更，再统一执行，最后总结修改点。
...
3. 根据待执行变更与已定稿创作大纲，调用 update_work_profile（如需要，含 content_format / media_modality），再 generate_content 执行。
