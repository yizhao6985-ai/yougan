...
const FORMAT_GUIDANCE: Record<ContentFormatId, string> = {
  ...
  podcast: "播客：对话感或独白结构，分段主题，适合音频节目大纲与口播稿。",
...
export const OUTLINE_FORMAT_HINTS: Record<ContentFormatId, string> = {
  note: "大纲建议包含：钩子、核心卖点、体验细节、行动号召。",
  short_post: "大纲建议包含：核心观点、支撑句、话题标签方向。",
  article: "大纲建议包含：标题方向、导语、分节主题、总结。",
  blog: "大纲建议包含：论点、论据、案例、结论。",
  novel: "大纲建议包含：人物、背景、冲突、情节节点、结局走向。",
  video_script: "大纲建议包含：开场钩子、分镜/段落、口播要点、结尾 CTA。",
  short_video: "大纲建议包含：3 秒钩子、核心信息、结尾引导。",
  podcast: "大纲建议包含：节目主题、分段话题、过渡语、结尾。",
  music: "大纲建议包含：情绪基调、段落结构、关键词/歌词方向。",
};

export function buildOutlineFormatHint(contentFormat: ContentFormatId | null | undefined) {
  if (!contentFormat) return "";
  return OUTLINE_FORMAT_HINTS[contentFormat] ?? "";
}
