 ...
 */
import type { FlatContentSpec, ProductionDepartment } from "#agent/schema.js";
...
const DEPARTMENT_BRIEF: Record<ProductionDepartment, string> = {
  ...
  design:
    "设计部：负责绘画创作、封面、插画、海报与配图；输出文生图提示词并生成图片。",
...
export function resolveIndustryContext(spec: FlatContentSpec): string {
  ...
  if (spec.content_topic) {
    parts.push(`创作主题「${spec.content_topic}」：需结合该领域受众关注点与常见内容形式。`);
  }
