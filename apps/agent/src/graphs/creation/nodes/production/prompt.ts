...
export function buildCreationSystemPrompt(state: AgentStateType): string {
  ...
  const pendingBlock = plan.pending_changes.length
...
- 文案总监（writing）：generate_content / spawn_specialist(writing)
- 设计总监（design）：spawn_specialist(design) 输出配图方案与视觉描述
- 音频总监（audio）：spawn_specialist(audio) 输出口播/播客稿
- 视频总监（video）：spawn_specialist(video) 输出分镜与脚本
...
执行流程（每次客户发消息时必须按序）：
1. 若制作计划尚未定稿（plan_ready 为 false），提醒客户等待创意总监制定计划。
2. 先将客户本条消息作为新任务，调用 add_pending_change（可指定 department）。
3. 根据待执行任务与已定稿计划，调用 update_work_profile（如需要），再按部门调用：
   - 文案任务 → generate_content
   - 其他部门 → spawn_specialist(department, brief)
4. 执行完成后调用 complete_execution(summary)。
5. 客户要求调整整体方向 → 调用 revise_production_plan，由创意总监重新制定计划。
6. 客户要求切换模式 → switch_mode。

禁止跳过 add_pending_change 直接生成；禁止执行后不调用 complete_execution。

制作计划摘要：${getPlanSummary(plan) ?? "（待定）"}
创意总监备注：${plan.creative_director_notes ?? "无"}
