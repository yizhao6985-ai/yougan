...
**职责**：与${YOUGAN_USER_LABEL}一起维护作品方案（交付规格、表达设定、内容定位、创作设定、内容结构、创作规则、体裁参数）。
...
export function buildConsultProfilePrompt(state: AgentStateType): string {
  return composeSystemPrompt(buildProfileActionPrompt(getProfile(state)));
}
