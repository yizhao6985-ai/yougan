...
function appendSetting(
  ...
  if (!trimmed) return null;
  if (
    profile.blueprint.settings.some(
      (item) =>
...
function replaceSettings(
  ...
  return {
    ...base,
    blueprint: { ...base.blueprint, settings: nextSettings },
  };
}
...
function hasProfilePatchInput(patch: ProfilePatch): boolean {
  ...
  if (patch.setting_updates?.length) return true;
  if (patch.setting_deletes?.length) return true;
...
function findGuardrailIndex(profile: WorkProfile, guardrailId: string): number {
  return profile.guardrails.findIndex((item) => item.id === guardrailId);
}

function findSegmentIndex(profile: WorkProfile, segmentId: string): number {
  return profile.blueprint.segments.findIndex((item) => item.id === segmentId);
}

function findSettingIndex(profile: WorkProfile, settingId: string): number {
  return profile.blueprint.settings.findIndex((item) => item.id === settingId);
}
...
export function applyProfilePatch(
  ...
  for (const settingId of patch.setting_deletes ?? []) {
    if (findSettingIndex(next, settingId) < 0) {
      warnings.push(`未找到创作设定 ${settingId}`);
      continue;
    }
    next = deleteSetting(next, settingId);
    if (!changes.includes("创作设定")) changes.push("创作设定");
  }

  for (const update of patch.setting_updates ?? []) {
    if (findSettingIndex(next, update.setting_id) < 0) {
      warnings.push(`未找到创作设定 ${update.setting_id}`);
      continue;
    }
    next = updateSetting(
      next,
  ...
  if (patch.settings_replace?.length) {
    ...
    const before = next.blueprint.settings.length;
    next = appendSettings(next, patch.settings_append);
    if (next.blueprint.settings.length > before) {
      changes.push("创作设定");
    }
  }
