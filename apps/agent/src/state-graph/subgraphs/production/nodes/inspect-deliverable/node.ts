/** 单任务交付物质检；不通过时供 after-inspect-deliverable 重试 */
...
import { retryDeliverable } from "./helpers/retry-deliverable.js";
...
export async function inspectDeliverableNode(
...
标准：内容完整、符合任务描述、无明显跑题或空洞套话。`;
  ...
  const retried = await retryDeliverable(state, task, feedback);
