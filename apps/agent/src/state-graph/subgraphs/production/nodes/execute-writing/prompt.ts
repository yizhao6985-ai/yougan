...
- direction 若含 **【下节起笔】**：必须从该点接续，禁止重开已结束场景；若含 **【禁止重复】**：本节开头勿复述上一节末尾刚写过的 beat（禁止换词、扩写式重述），同一场景/人物/线索可随情节继续出现
- direction 若含 **【本节止于】**：本节正文须收束于该情节点，勿越界写下节内容
- 正文避免互联网黑话与空泛套话（如「赋能」「抓手」「链路」「颗粒度」「闭环」等），用具体、可感的表达`;
}
...
- 禁止重开前序已写场景；禁止在段首换词重复前序末尾刚完成的动作句或情绪定格（前序元素可继续推进，非整节禁现）\n`
    : "";
