...
function formatUnprocessedJobs(jobs: UnprocessedReferenceJob[]): string {
  ...
  return jobs
    .map((job) => {
      ...
      return `- ${tool}｜${label}｜${name ? `文件名：${name}｜` : ""}url：${job.asset_url}`;
    })
    .join("\n");
}
...
**职责**：对尚未完成分析的参考资源执行预处理（读取文本、感知图片、转写音视频等），写入 analysis；intent 保持待确认。

**流程**
- 下方「待预处理」列表中的每条资源须调用且仅调用一次对应媒介工具（asset_url 原样传入）
- 全部处理完成前不要结束本轮；无 tool_calls 表示预处理已结束
- 禁止修改 intent、禁止删除参考、禁止向${YOUGAN_USER_LABEL}回复（后续节点负责）

**工具对照**
- 文本 → preprocess_reference_text
- 图片 → preprocess_reference_image
- 音频 → preprocess_reference_audio
- 视频 → preprocess_reference_video
