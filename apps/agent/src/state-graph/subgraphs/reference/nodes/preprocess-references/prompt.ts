...
import { isSupportedReferencePreprocessKind } from "../../helpers/reference-preprocess-kind.js";
...
**职责**：对尚未完成分析的参考资源执行预处理（读取文本、感知图片/音频），写入 analysis；intent 保持待确认。视频素材暂不支持自动分析，系统会自动跳过。

**流程**
- 下方「待预处理」列表中**文本、图片与音频**须调用且仅调用一次对应媒介工具（asset_url 原样传入）
